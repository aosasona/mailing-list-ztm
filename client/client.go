...
func getEmailBatch(client pb.MailingListServiceClient, count, page int) {
	...
	res, err := client.GetEmailBatch(
		ctx,
		&pb.GetEmailBatchRequest{Count: int32(count), Page: int32(page)},
	)
	if err != nil {
		log.Fatalf(">> error: %v", err)
	}
	...
	resLength := len(res.EmailEntries)
	for i := 0; i < resLength; i++ {
		log.Printf(">> [%v of %v]: %s", i+1, resLength, res.EmailEntries[i])
	}
}
...
func main() {
	...
	newEmail := createEmail(client, "jackryan@mail.com")
